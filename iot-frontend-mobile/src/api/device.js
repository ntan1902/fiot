import { API_DEVICE, API_ENTITY_SERVICE, BASE_URL } from '../config/setting'
...
export const DeviceApi = {
  ...
  getCredentialsById: async (id) => {
    return AxiosApi.get(
      `${API_ENTITY_SERVICE}/${API_DEVICE}s/credentials/${id}`
    )
  },
  ...
  updateAttributes: async (id, data) => {
    return AxiosApi.put(
      `${API_ENTITY_SERVICE}/${API_DEVICE}s/attributes/${id}`,
      data
    )
  },
  ...
  rpcRequest: async (id, data) => {
    return AxiosApi.post(
      `${API_ENTITY_SERVICE}/${API_DEVICE}s/rpc/request/${id}`,
      data
    )
  },
  ...
  getGatewayDevices: async (id) => {
    return AxiosApi.get(
      `${API_ENTITY_SERVICE}/${API_DEVICE}s/gateway-${API_DEVICE}s/${id}`
    )
  },
}
